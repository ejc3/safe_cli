...
- `headers`: an explicit map of fixed header values a verb sends that no flag supplies (`account set` → `{"x-pending-activation": "false"}`), analogous to `query`; every name must be one the op declares. Headers the op already fixes in `header_values` (`app-name=VSF`) are auto-sent by the engine and need no entry, and the identity/trace headers are filled by the engine.
