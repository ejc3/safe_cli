...
"pauseInternet": {
  ...
  "cli": {
    ...
    "flags": [
      ...
      {"name": "indefinite", "type": "bool", "default": false, "maps_to": "body:$indefinite", "excludes": ["for"],
       "help": "Pause until you run `pause-internet resume` (sends untilIUnpause=true and no pauseSchedule)."},
...
- `body_template`: the descriptor's `body_example` with `$vars`; unreferenced example fields are kept as fixed values (that is how `MAPPVersion`, `editSource`, `productType` stay baked). A test asserts every `$var` is either a flag's `maps_to` or a `resolve` entry.
...
**account** [account, user_setting, legal] — `show` <- getAccountDetails [core]; `set --family-name --timezone` <- updateFamilyNameOrTimeZone (reads current so the untouched field is not nulled) [common]; `leave --confirm` <- deleteMyself [long-tail]; `enable|disable keep-signed-in` <- user_setting.updateUserSettings [common]; `request-export`, `export-link` <- legal.* [long-tail]. call: selfRemoveProfile.
