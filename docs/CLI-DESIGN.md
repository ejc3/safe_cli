...
- From the service-id the CLI resolves, internally and per invocation, everything an op's body/query/path needs: `profileId` (`userProfileId`), `deviceId` and `pairingStatus`, all from one `account.getAccountDetails` read (which carries no timezone — see the `--timezone` rule in §3) (the same call `members` makes today; cached for the process). The user never types profileId/deviceId. Examples that need it: `pause_internet.pauseInternet` (profileId, serviceId, deviceId in the body), `calls_and_texts.deleteContactFromTheList` (profileId, deviceId as query), `app_block.getBlockedApps` (`{deviceId}` path placeholder), `location.fetchHistory` (`profileId` query), `restricted_usage.addLimit` (profileId/deviceId in body).
...
| `enable` / `disable` | flip a boolean feature; when the area has more than one such feature the feature is a group with the verbs under it (`website safe-search enable`, `calls trusted-only enable`) |
...
The descriptor stays the single source of truth. Each op gains a `cli` entry — one verb block, or a LIST of verb blocks when one operation backs several verbs (`content_filter.updateSubcategory` backs `filter block` and `filter allow`; every enable/disable and approve/decline pair likewise). Sibling blocks share the op's method, path and templates and differ by `constants` (`{"enabled": true}` vs `{"enabled": false}`), defaults or flags; `alias_of` and `call_only` are single entries. The kong tree is generated from it.
...
"pauseInternet": {
  ...
  "cli": {
    ...
    "flags": [
      ...
      {"name": "timezone", "type": "tz", "default": "$local.timezone", "maps_to": "body:$tz", "transform": "tz_short",
       "help": "Short zone code (EST, PST) the pause timing is computed in. Default: this machine's local zone."}
    ],
...
- `flags[]`: `name`, `type` (`string|int|float|bool|enum|duration|date|datetime|tz|list`), `enum[]`, `default` (a literal, `$local.timezone` — the one resolver variable a default may name, on a `tz`/`string` flag — or a lookup such as `$lookup:account.getAccountDetails::familyName` so an untouched field is resent from its current value; any other resolver variable belongs in `resolve` and the template), `required`, `repeatable`, `excludes[]`, `maps_to` (`body:$var` into `body_template`, `query:<name>`, `header:<name>`, `path:<placeholder>`, a `filter:<field>` destination that keeps the flag out of the request and filters the response client-side, or a `find:<field>` destination (string flags only) that keeps the objects whose field contains the text, case-insensitively, under the groups that contain them — `apps list --find tiktok` (the probe's agent had no way to turn a name into an id) — `location where --child` on the account-scoped dashboard, `calls list --list` — so an optional selector never changes the request's scope), `transform` (a fixed registry in Go: `pause_schedule`, `tz_short`, `iso_micro`, `epoch_ms`, `day3_lower`, `day3_title`, `bool01`, `allow_block_ab`, `preset_group` (`none|young-child|child|teen` → createGroupPolicy's groupId 1..4); `weekday_ints` joins when postScheduleAlert's weekDays integer convention is observed on the wire — every captured example has `weekDays: []`), `help`. `excludes[]` may only name flags that have no `default`: a defaulted flag is always populated, so an exclusion against it could not tell "only `--indefinite`" from "`--indefinite --for 30m`" and would either reject every indefinite pause or none; precedence over a defaulted flag is expressed with `nulls[]` instead (the nulled flag's `"$var?"` property is omitted and its help says it is ignored). A verb-level `at_least_one: [flags]` requires at least one of the named optional flags (`account set`). A flag has exactly one of `maps_to` or `spreads_to`: `spreads_to: ["body:$a", "body:$b"]` lets one semantic flag populate several body fields through a *structured* transform that returns one value per destination — `downtime add --mode block|alert` spreads to `blockContent` and `alertOn` via `mode_block_alert` (`block` → `{blockContent: true, alertOn: false}`, `alert` → the inverse), the wire-verified mutually exclusive pair. Parse checks every destination is a template var and the transform is one of the structured ones; the engine checks the transform returned exactly those vars. Two flags may map to the same body var only if each `excludes` the other (`screen-time set --weekdays` spreads over mon..fri while `--mon` targets one day; they exclude each other), so one template value never has two live sources. Every `{placeholder}` in the op's path must have a source at Parse: a flag's `path:<placeholder>` or, for `child`/`device` verbs, one of the target's ids (`{deviceId}`, `{profileId}`, `{serviceId}`) that the engine fills. Dependent flags: `requires: [flags]` on a flag means that when it is given the named flags must be given too (`--lat` requires `--lon`), and a verb-level `one_of: [["lat","lon"],["address"]]` means exactly one alternative group must be fully given (`pick-me-up request --lat --lon | --address`, `watch validate --imei --iccid | --mdn`); an invocation satisfying none or more than one group is rejected before any request, and Parse checks every name is a declared flag.
- `body_template`: the descriptor's `body_example` with `$vars`. It is strict JSON: every variable is written as a JSON string (`"$child.profileId"`, `"$for?"`) so the template parses for validation, and the engine substitutes the typed value (number, bool, string) at build time. A property written as `$var?` is **omitted from the body entirely** when its variable is unset or nulled by another flag's `nulls[]` — this is how `--indefinite` sends `untilIUnpause=true` with no `pauseSchedule`, matching the wire-verified indefinite body (the app sends the field as absent, not empty); `excludes[]` only rejects flag combinations and carries no omission semantics on its own. Every retained field must be **classified**: mapped (a flag's `$var`), resolved (a `resolve` entry), or a declared constant in `constants` (that is how `MAPPVersion`, `editSource`, `productType` stay baked — explicitly, by name). Any `body_example` field that is none of the three is **rejected** by the descriptor test, because examples carry request-specific sample data, not only protocol constants: `geofence.createDeviceGeofenceSettings`' sample `geofenceId` must be dropped (create must omit the server-assigned id), and `invite.sendInvite`'s unrelated pet, contact and Wi-Fi fields must never ship from `member invite`. The test asserts every `$var` is a flag's `maps_to` or a `resolve` entry, every constant is declared, and no unclassified field remains. A `repeatable` flag's var must sit inside a single-element array template — `"domains":[{"status":"$status","url":"$url"}]` for `website block --url a.com --url b.com` — and the engine expands that element once per value, sharing the element's other vars; a repeatable var anywhere else is rejected at Parse.
- `query`: an explicit map of query-parameter values — constants and flag-mapped variables — because the descriptor's name-only `query[]` cannot carry a value and several zero-flag verbs need one that today lives only in prose or op metadata: `filter set` needs `{"categorySupported": "v6"}`, `places list` needs `{"strategy": "NotNull"}`, `calls log` maps `{"startDate": "$since", "endDate": "$until"}`. A value may also be a resolver variable (`{"profileId": "$child.profileId", "deviceId": "$child.deviceId"}` for the child reads `location history` and `calls list`), the same vocabulary as body variables and validated the same way. Validated like the body template: every name in the op's `query[]`/`required_query` must be a flag's `maps_to: query:<name>` target, a `query` constant or resolver variable, or explicitly marked optional; a required name with no source fails the descriptor test. A flag's `query:<name>` may name a parameter declared by the verb's op or by any of its `select` branch ops (`calls log --number` → `otherPartyMdn` exists only on the specific-contact op); the engine sends only the parameters the chosen op declares. Values already baked into the op's path string (`?activityType=call,text&timezone=preferred`) stay there and are not re-declared.
- `headers`: an explicit map of fixed header values, or resolver variables (`{"timezone": "$local.timezone"}` for ops that declare a contextual timezone header, as `chores list` → `todo.invoke` does), that a verb sends when no flag supplies them (`account set` → `{"x-pending-activation": "false"}`), analogous to `query`; every name must be one the op declares. Headers the op already fixes in `header_values` (`app-name=VSF`) are auto-sent by the engine and need no entry, and the identity/trace headers are filled by the engine.
- `resolve[]`: the variables the runtime must fill: `$child.serviceId|profileId|deviceId|pairing`, `$self.serviceId|profileId`, `$account.id`, `$local.timezone`, `$now.epochMs`, `$uuid`, and `$lookup:<entity>.<op>:<key>=<flag>:<field>` for enrichment (e.g. `$lookup:content_filter.getCategories:id=category:name`); the op may be followed by `/<field>` to search only that top-level subtree of the read (`$lookup:content_filter.getCategories/Apps & websites:id=app:name` keeps `apps block` off the content categories); a `^field` reads the matched record's enclosing object instead, because getCategories nests each subcategory `{id, name, enabled}` under its category `{id: "GAM", categoryId, name, subCategories}` and updateSubcategory wants the parent's `id` (as categoryShortName) and `categoryId` next to the subcategory's own name. The unkeyed form `$lookup:<entity>.<op>::<field>` (empty key) reads the target's singleton record: `screen-time set` reads `schedules.getScreenTimeData` for `screenTimeLimitId`, whose presence drives the `exists:` condition and whose value fills the PUT; `account set` reads the current `familyName` the same way (the singleton is the first object in the response that carries the field, so a wrapped read such as getAccountDetails' `{"accounts":[{...}]}` works). A lookup runs before the verb's own `--confirm` guard, so Parse requires every lookup op — in `resolve`, a flag default, or an `exists:` condition — to be a read-only GET that is not `destructive`; within one invocation each lookup is read once, so an `exists:` condition and the `resolve` entry naming the same lookup share a snapshot.
- `output`: `{table: [fields]}` for the default table, and `pick: <field>` to project the response to one top-level field before output (`apps list` answers with getCategories' "Apps & websites" list, not the whole document).
- `ok_on`: `[{status, contains, result}]` — a documented error response the verb reports as a successful no-op (`pause-internet resume` on an already-unpaused device answers 500 "Device already unpaused" and is reported as "already resumed"), so a retry is idempotent; Parse requires a 4xx/5xx status, a body text and a result.
- `select`: an ordered list of `{"when": <condition>, "op": "<entity>.<op>"}` for a verb whose operation depends on how it is invoked; the first matching condition wins and the block's own op is the default. Conditions: `child` (`--child` was given — `alerts settings show|set`, `emergency-contacts list`), `flag:<name>` (the flag was explicitly given — `calls log --number` switches from `getCallAndTextActivityListV7` to `getCallAndTextSpecificContactActivityListV7`), and `exists:$lookup:<entity>.<op>:<key>=<flag>:<field>` (a lookup found a record — `screen-time set` chooses `putScreenTimeData` when a `screenTimeLimitId` exists, else `postScreenTimeData`). Every op named must exist, every `flag:` must be a declared flag, and every `exists:` a valid lookup. A flag whose `query:` param only a branch op declares must be reachable whenever it is given: it is that branch's `flag:` condition, it `requires` the flag that is, or the branch is the `child` one — otherwise the flag alone would select the base op and its value would have to be dropped, so Parse rejects the block; at run time an explicitly given flag the chosen op does not take is refused naming the selector (`--alt is only accepted with --child`), never dropped. A branch may also override `target`, `body_template`, `query`, `constants` and `resolve` (unspecified fields inherit the verb's), because a branch can be a different request contract: `alerts settings set --child` posts unversioned keys (`objectionableAlerts`) to `dashboard.postReportSettings` under the child's service-id header, while the account branch posts versioned keys (`newContactAlertV2`) to `notifications.updateReportSettings` under the caller's. Each branch is validated at Parse as a complete verb against its own op (body-ness, query names, path placeholders), so selecting an op can never apply the base template to an unrelated route. When a verb's base target is `account`/`self` and a branch's target is `child`, the generator unions the targets and emits `--child` as OPTIONAL for that verb (`alerts settings show [--child]`); selection is evaluated on whether it was given, and the chosen branch's target rule then applies. Selection is declared here, never a runtime special case.
...
Closure: the schema is complete for a surface once every verb in that surface has a `cli` block that validates. Gaps found while writing those blocks are fixed in the same PR, with a RED test, not by another design review; the design document is amended in that PR.

## 5. The command tree

Format: `verb <- entity.op [priority]`. Long tail grouped at the end of each area as `call: ...`. Ops marked unavailable in the descriptor (79) are not generated (section 8).
...
**filter** [content_filter] — `show` <- getFilterContent [core]; `set --preset none|young-child|child|teen` <- createGroupPolicy [core]; `block --category <id>` <- updateSubcategory enabled=true [core]; `allow --category <id>` <- updateSubcategory enabled=false [core]; `categories [--search-engines]` <- getCategories [core] (the further content categories — Downloads, News, Search engines, Shopping, Sports, individual websites — and every app, with ids; `show` carries the objectionable/safe-search/security ones with their ids, so the two reads split the id space: `block|allow` read `show`'s op and switch to `categories`' op through an `exists:` branch when the id lives there); `presets` <- getAgeGroupMetaData [common]. call: getSubCategoriesDetail, getParentalControls, setCFCategories (bulk form of block/allow). Objectionable-alert settings live in `alerts settings`.

**apps** [app_block, content_filter.getCategories] — `list` <- content_filter.getCategories (the Apps & websites groups, enabled=true meaning blocked; app_block.getBlockedApps answers 403 to a guardian token — a device-side sync read, marked unavailable) [core]; `block --app <id>` <- blockApp enabled=true [core]; `allow --app <id>` <- blockApp enabled=false [core]. app_management.getAppStatus/updateAppStatus also answer 403 for a phone child (managed-device app management) and are marked unavailable, so there is no `status`/`set`. call (device-originated): sendAccessibilityStatus, sendBlockStatus, installed_apps.sendInstalledApps (unavailable).
...
Not generated (descriptor `unavailable`, 79 ops): pet_tracker (22), messaging (19), tamper telemetry (14), pairing device/Gizmo reads (6), video_calling (3), wearable device-side (3), family_line SPC-gated management (3), activity_tracking comms feed (2), device_settings device-side reads (2), app_management managed-device app status (2), app_block device-side sync (1), installed_apps (1), real_time_tracking session read (1). Reachable via `call --force`. Clearing `unavailable` is not enough by itself: an op is generated only from its `cli` mapping, so enabling one means adding (or un-dormanting) its mapping, which the coverage test then requires; dormant mappings may be written ahead of time when a product's shapes are known.
...
Flags:
      ...
      --timezone=TZ        Short zone code (EST, PST) the timing is computed in (default: local timezone).
```
...
Ops (all live-verified on a real paired device, block→verify→allow, 2026-09-08): `filter show` <- `content_filter.getFilterContent` (app-name: VSF header auto-sent; carries the objectionable, safe-search and security subcategories WITH ids); `filter set` <- `createGroupPolicy` (`--preset none|young-child|child|teen` -> groupId 1..4 via the `preset_group` transform; help says a preset replaces current subcategory filters); `filter block|allow` <- `updateSubcategory` with `enabled` true/false, `name`/`categoryId`/`categoryShortName` enriched by keyed lookups — the parent category's fields through `^categoryId`/`^id` — from `getFilterContent` by default and from `getCategories` when an `exists:` branch finds the id there (Downloads, News, ... and apps live only in that read); `filter categories` <- `getCategories` (`--search-engines` sets include-safesearch); `filter presets` <- `getAgeGroupMetaData`. `apps list` <- `getCategories` (app_block.getBlockedApps is unavailable: 403 to a guardian token); `apps block|allow` <- `blockApp`, same enrichment. `website list` <- `web_and_apps.getWebsites2`; `website block|allow` <- `postWebsites` (status b/a, repeatable `--url`); `website remove --entry-id` <- `deleteWebsite` (profileDomainId from `website list`); `website safe-search enable|disable` (the first generated group: a third command level whose help is the areas map's `"website safe-search"` entry).
...
- Verbs: `preset` -> `filter set --preset`; `rename` -> `member set --name` / `device set --name`; `clear` -> `remove`; `update` -> `set`; `respond` -> `approve|decline`; `safe-search-on/off` -> `safe-search enable|disable`; `block-private`/`trusted-only` -> `enable|disable block-unlisted|trusted-only`; `where` kept as the single "where is my kid" verb and `getDashboardDetails` is its only op. A `calls block` shorthand was rejected: it would be asymmetric with `remove --list` and `add --list` is unambiguous.
...
- Same-verb, op-by-target: `alerts settings show|set` and `emergency-contacts list` pick the account-wide or per-child op by whether `--child` is present (`select` with the `child` condition); `calls log --number` and `screen-time set` use the `flag:` and `exists:` conditions.
- Stay on `call` by design: device-originated telemetry (tamper.*, app_block.sendBlockStatus/sendAccessibilityStatus, schedules.putAppUsageStats/updateSchedulerRunStatus, flightdetection.updateFlightDetectionStatus, location.postGeofenceViolationEvent/postLocationTamper), SDK/auth plumbing (identity.*, pubnub.*, config.*, driving_insights.getAuthCode, calendar_sync.getGoogleCalendarTokens, age_verification keys), child-side asks (askForMoreScreenTime, postAppTimeLimit), and the 79 unavailable ops. `multipart` ops (2) remain refused by `call` and get no verb.
...
5. **Unavailable ops.** Recommendation: not generated; `call --force` only. Clearing `unavailable` in the descriptor does not by itself emit them: enabling an op also means adding (or un-dormanting) its `cli` mapping, and the coverage test requires it from then on.
