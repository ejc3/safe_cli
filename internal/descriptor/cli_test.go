...
func TestCLIValidationRejects(t *testing.T) {
	cases := []struct {
		...
		{"two flags to one placeholder", `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s","flags":[{"name":"a","type":"string","required":true,"maps_to":"path:thing","help":"h"},{"name":"b","type":"string","required":true,"maps_to":"path:thing","help":"h"}]}`, `"takes_body":false,"path":"/x/{thing}"`, "already mapped"},
		...
		// Codex #70 round 10 / #69: a path placeholder's flag is always present; a requires edge
		// whose target excludes the requirer is a contradiction; a lookup's key flag carries a
		// literal default at most (a lookup-defaulted key would need a lookup to key a lookup).
		...
		// Codex #70-14: unused constants and resolve entries are judged per effective contract
		// (the base, and each select branch with its overrides), never across the union.
		{"constant unused by a select branch", `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s","body_template":"{\"x\":\"$x\",\"k\":\"v\"}","constants":{"k":"v"},"select":[{"when":"flag:alt","op":"pause.other2","body_template":"{\"x\":\"$x\",\"k\":\"v\"}","constants":{"k":"v","typo":"z"}}],"flags":[{"name":"x","type":"string","required":true,"maps_to":"body:$x","help":"h"},{"name":"alt","type":"string","maps_to":"query:alt","help":"h"}]}`, "", "never used"},
		{"base resolve entry used only by a branch template", `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s","body_template":"{\"x\":\"$x\"}","resolve":["$lookup:pause.other::n"],"select":[{"when":"flag:alt","op":"pause.other2","body_template":"{\"x\":\"$x\",\"n\":\"$lookup:pause.other::n\"}"}],"flags":[{"name":"x","type":"string","required":true,"maps_to":"body:$x","help":"h"},{"name":"alt","type":"string","maps_to":"query:alt","help":"h"}]}`, "", "never used"},
		{"branch resolve entry used only by a sibling branch", `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s","body_template":"{\"x\":\"$x\"}","select":[{"when":"flag:alt","op":"pause.other2","body_template":"{\"x\":\"$x\"}","resolve":["$lookup:pause.other::n"]},{"when":"flag:alt2","op":"pause.other2","body_template":"{\"x\":\"$x\",\"n\":\"$lookup:pause.other::n\"}","resolve":["$lookup:pause.other::n"]}],"flags":[{"name":"x","type":"string","required":true,"maps_to":"body:$x","help":"h"},{"name":"alt","type":"string","maps_to":"query:alt","help":"h"},{"name":"alt2","type":"string","maps_to":"query:lon","help":"h"}]}`, "", "select[0]: resolve entry"},
		{"inherited resolve entry unused by a branch template", `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s","body_template":"{\"x\":\"$x\",\"n\":\"$lookup:pause.other::n\"}","resolve":["$lookup:pause.other::n"],"select":[{"when":"flag:alt","op":"pause.other2","body_template":"{\"x\":\"$x\"}"}],"flags":[{"name":"x","type":"string","required":true,"maps_to":"body:$x","help":"h"},{"name":"alt","type":"string","maps_to":"query:alt","help":"h"}]}`, "", "select[0]: resolve entry"},
	}
...
func TestCLIExactResolveLookupAndPathAccepted(t *testing.T) {
	cli := `{"area":"a","verb":"v","priority":"core","target":"child","summary":"s",
	  ...
	  "flags":[{"name":"cat","type":"int","required":true,"maps_to":"body:$cat","help":"h"},{"name":"device-id","type":"string","maps_to":"path:deviceId","help":"h"}]}`
