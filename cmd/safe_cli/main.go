...
type CLI struct {
	...
	Members  membersCmd  `cmd:"" help:"List family members with the ids you target: pass a child's SERVICE-ID as --child to child-scoped verbs (or as --service-id to call). Rows: guardians first, then children with PAIRED before UNPAIRED, then by name. Start here."`
